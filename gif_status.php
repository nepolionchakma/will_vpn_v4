<?php 
    $action=$_GET['action'];
    $page_title=ucfirst($action).' GIF Status';
    $active_page="status";
    ...
    if(isset($_POST['btn_submit']) AND $action=='add')
    {
        ...
        if($file_size > $settings_details['gif_file_size']) {

            $gif_size_msg=str_replace('###', $settings_details['gif_file_size'], $client_lang['gif_size_msg']);

            $_SESSION['class']='error';
            $_SESSION['msg']=$gif_size_msg;
            if(isset($_GET['redirect']))
            {
              header("Location:gif_status.php?action=add&redirect=".$_GET['redirect']);
            }
            else{
              header( "Location:gif_status.php?action=add");
            }
            exit;
        }
        ...
        $image_file=rand(0,99999)."_gif_status.".$ext;
        ...
        if(isset($_POST['notify_user'])){
          ...
          $fields = array(
              ...
              'data' => array("foo" => "bar","type" => "single_status","status_type" => "gif","id" => $last_id,"external_link"=>false),
        ...
        if(isset($_GET['redirect']))
        {
          ...
          header( "Location:manage_gif_status.php");
        }
        exit; 
    }
    else if(isset($_POST['btn_submit']) AND ($action=='edit' OR isset($_GET['edit_id'])))
    {
        ...
        if (!empty($_FILES['image_file']['name'])) {
            ...
            if($file_size > $settings_details['gif_file_size']) { 
                ...
                if(isset($_GET['redirect']))
                {
                  header( "Location:gif_status.php?edit_id=".$_POST['edit_id']."&action=edit&redirect=".$_GET['redirect']);
                }
                else{
                  header( "Location:gif_status.php?edit_id=".$_POST['edit_id']."&action=edit");
                }
                exit;
            }
  ...
  <div class="col-md-12">
    <?php
      if(isset($_GET['redirect'])){
        ...
        echo '<a href="manage_gif_status.php" class="btn_back"><h4 class="pull-left" style="font-size: 20px;color: #1ee92b"><i class="fa fa-arrow-left"></i> Back</h4></a>';
      }
    ?>
    <div class="card">
        ...
        <form action="" method="post" class="form form-horizontal" enctype="multipart/form-data">
          ...
          <div class="section">
            <div class="section-body">
              ...
              <div class="form-group">
                <label class="col-md-3 control-label">GIF Image Layout </label>
                <div class="col-md-6">
              ...
              <div class="form-group">
                <label class="col-md-3 control-label">Select Image :-
                  ...
                  <p class="control-label-help">(<strong>Note:</strong> Maximum <strong><?=$settings_details['gif_file_size']?>MB</strong> file size)</p>
                </label>
                <div class="col-md-6">
                  <div class="fileupload_block">
                    <input type="file" name="image_file" value="fileupload" id="fileupload" accept=".gif" onchange="fileValidation()" <?=(!isset($_GET['edit_id'])) ? 'required' : ''?>>
...
<script type="text/javascript">
  ...
  function fileValidation(){
    ...
    if(!allowedExtensions.exec(filePath)){
        ...
        if(filePath!=''){
          swal({title: 'Invalid!',text: 'Please upload file having extension .gif only.', type: 'warning'});
          fileInput.value = '';
          return false;  
        }
        else{
        ...
        if (fileInput.files && fileInput.files[0]) {
            ...
            var required_file_size=parseFloat('<?=$settings_details['gif_file_size']?>');
            ...
            if(file_size <= required_file_size)
            {
              ...
              fileInput.value = '';
              var msg="<?=str_replace('###', $settings_details['gif_file_size'], $client_lang['gif_size_msg']);?>";
