...
$sql1="SELECT tbl_reports.*, tbl_users.`name`, tbl_users.`user_image` FROM tbl_reports, tbl_users WHERE tbl_reports.`post_id`='$id' AND tbl_reports.`report_type`='video' and tbl_users.`id`=tbl_reports.`user_id` ORDER BY tbl_reports.`report_on` DESC";
